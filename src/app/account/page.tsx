...
export default function AccountPage() {
  ...
  const handleLogout = () => {
    // Clear persisted cart and wishlist from localStorage
    localStorage.removeItem("zagull-cart");
    localStorage.removeItem("zagull-wishlist");
    // Redirect to home (Supabase sign-out will go here later)
    router.push("/");
  };
  return (
          ...
          <div className="flex flex-col md:flex-row gap-12">
            ...
            <aside className="w-full md:w-64 space-y-2">
              <div className="p-6 bg-white rounded-2xl border border-border-light shadow-sm text-center mb-8">
                <div className="w-20 h-20 bg-accent-forest rounded-full mx-auto flex items-center justify-center text-white text-3xl font-serif mb-4">
                  Z
                </div>
                <h2 className="text-lg font-serif text-text-primary">Zain Ahmed</h2>
                <p className="text-xs text-text-secondary">zain@example.com</p>
              </div>
            ...
            <div className="flex-grow space-y-8">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {[
                  ...
                  { label: "In Wishlist", value: "8", icon: Heart },
                  ...
                  <div key={stat.label} className="bg-white p-6 rounded-2xl border border-border-light shadow-sm text-center">
                    <stat.icon className="w-6 h-6 text-accent-forest mx-auto mb-3 opacity-20" />
              ...
              <div className="bg-white rounded-3xl border border-border-light shadow-sm overflow-hidden">
                <div className="p-8 border-b border-border-light flex justify-between items-center">
                  ...
                  <Link href="/account/orders" className="text-xs text-accent-forest font-bold hover:underline">View All</Link>
                </div>
                <div className="p-8 text-center py-20">
                  <Package className="w-12 h-12 text-text-secondary/20 mx-auto mb-4" />
                  <p className="text-text-secondary italic">You haven't placed any orders yet.</p>
                  <Link href="/shop">
                    <Button className="mt-6 bg-accent-forest">Start Shopping</Button>
                  </Link>
                </div>
              </div>
            </div>
